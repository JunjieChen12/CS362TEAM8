...
class TaskwiseTestCase(unittest.TestCase):
    ...
    def test_addTask_withPastDate(self):
        """test the system behavior with a overdue task. """

        with app.app_context():
            past_time = datetime.now() - timedelta(minutes=10)
            past_time_str = past_time.strftime('%Y-%m-%d %H:%M')

            task = Task(
                name="Overdue Task", 
                deadline=past_time_str,
                is_completed=False
            )
        ...
        with app.app_context():
            ...
            deadline_as_dt = datetime.strptime(fetched_task.deadline, '%Y-%m-%d %H:%M')

            self.assertTrue(deadline_as_dt < datetime.now(), "The system should show that the task has expired.")
